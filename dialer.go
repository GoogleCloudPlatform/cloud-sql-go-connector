...
import (
	...
	"cloud.google.com/go/cloudsqlconn/internal/csqlgrpc"
...
type Dialer struct {
	...
	sqlDataClient   csqlgrpc.SqlDataServiceClient
}
...
func (d *Dialer) initSQLDataClient() (csqlgrpc.SqlDataServiceClient, error) {
	...
	sqlDataConn, err := grpc.NewClient(d.sqlDataEndpoint,
...
func (d *Dialer) connectSQLDataService(ctx context.Context, cn instance.ConnName, cfg dialConfig, startTime time.Time) (conn net.Conn, err error) {
	...
	err = stream.Send(&csqlgrpc.StreamSqlDataRequest{
		LocationId: fmt.Sprintf("locations/%s", cn.Region()),
		MessageType: &csqlgrpc.StreamSqlDataRequest_ConnectionSettings{
			ConnectionSettings: &csqlgrpc.ConnectionSettings{
				Target: &csqlgrpc.ConnectionSettings_InstanceId{
					InstanceId: &csqlgrpc.InstanceId{
						Instance: fmt.Sprintf("projects/%s/instances/%s", cn.Project(), cn.Name()),
					},
				},
			},
		},
...
type streamConn struct {
	stream     csqlgrpc.SqlDataService_StreamSqlDataClient
...
func (c *streamConn) Read(b []byte) (n int, err error) {
	...
	m, ok := msg.Type.(*csqlgrpc.StreamSqlDataResponse_Payload)
	if !ok {
...
func (c *streamConn) Write(b []byte) (n int, err error) {
	err = c.stream.Send(&csqlgrpc.StreamSqlDataRequest{
		LocationId: c.locationID,
		MessageType: &csqlgrpc.StreamSqlDataRequest_Payload{
			Payload: &csqlgrpc.ClientPayload{
				Data: b,
			},
		},
	})
...
func (d *Dialer) Close() error {
	...
	if d.sqlDataConn != nil {
		// Best effort close with debug logging of error
		if err := d.sqlDataConn.Close(); err != nil {
